...
export default function ReportVentesDepot({ invoices, stats, filters, clients, depots }: Props) {
    const [dateFrom, setDateFrom] = useState<string>(filters?.date_from || '');
    const [dateTo, setDateTo] = useState<string>(filters?.date_to || '');
    const [clientId, setClientId] = useState<number | string>(filters?.client_id || 'all');
    const [depotId, setDepotId] = useState<number | string>(filters?.depot_id || 'all');
    ...
    const columns = useMemo<ColumnDef<Invoice>[]>(() => [
        ...
        },
        {
            ...
            header: 'Dépôt',
            cell: ({ row }) => <span className="text-xs">{row.original.depot?.name || '-'}</span>,
        },
        {
    ...
    const handleFilter = () => {
        router.get(toUrl(reportsActions.default.venteDepot()), {
            date_from: dateFrom,
            date_to: dateTo,
            client_id: clientId === 'all' ? '' : clientId,
            depot_id: depotId === 'all' ? '' : depotId,
        }, { preserveState: true });
    };

    const handleDownload = () => {
        const url = new URL(toUrl(reportsActions.default.venteDepot.download()), window.location.origin);
        ...
        if (depotId && depotId !== 'all') {
            url.searchParams.append('depot_id', depotId.toString());
        }
    ...
    return (
        <>
            <Head title="Rapport Ventes Dépôt" />

            <div className="flex h-full flex-1 flex-col gap-4 p-4">
                <div className="flex items-center justify-between">
                    <h1 className="text-2xl font-bold text-foreground font-mono uppercase tracking-tight">Rapport des ventes sur dépôt</h1>
                ...
                <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-5">
                        ...
                        <div className="space-y-2">
                            <Label>Dépôt</Label>
                            <Select value={depotId.toString()} onValueChange={setDepotId}>
                                <SelectTrigger>
...
ReportVentesDepot.layout = {
    breadcrumbs: [
        { title: 'Rapports', href: '#' },
        { title: 'Vente dépôt', href: toUrl(reportsActions.default.venteDepot()) },
    ],
};
