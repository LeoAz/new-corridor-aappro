    ...
    <div class="container">
        <div class="header">
            <div class="header-left">
                ...
                <div class="service-name">Bamako - MALI | SERVICE FACTURATION (CHARGEMENT)</div>
        ...
        <div class="billing-section">
            <table class="billing-grid">
                <tr>
                    ...
                    <td class="billing-col" style="border: none;">
                        <div class="section-title">RÉSUMÉ DES VÉHICULES:</div>
                        @foreach($vehicleCounts as $product => $count)
                            <div class="summary-item">
                                <strong>{{ $product }}:</strong> {{ $count }} véhicule(s)
                            </div>
                        @endforeach
                    </td>
                </tr>
            ...
            <tbody>
                @foreach($invoice->items as $item)
                    <tr>
                        ...
                        <td class="text-right">{{ number_format($item->quantity_delivered, 2, ',', ' ') }} L</td>
        ...
        <div class="totals-section">
            <div class="total-row">
                ...
                <span class="total-value">{{ number_format($invoice->total_amount + ($invoice->total_missing * ($invoice->items->first()->unit_price ?? 0)), 0, ',', ' ') }} CFA</span>
            </div>
            @if($invoice->total_missing > 0)
            <div class="total-row">
                <span class="total-label">Déduction Manquant ({{ number_format($invoice->total_missing, 2, ',', ' ') }} L)</span>
                <span class="total-value" style="color: #b91c1c;">- {{ number_format($invoice->total_missing * ($invoice->items->first()->unit_price ?? 0), 0, ',', ' ') }} CFA</span>
            </div>
            @endif
        ...
        <div class="footer">
            <p>CORRIDOR APPRO &bull; Bamako, Mali &bull; Facture officielle de chargement</p>
